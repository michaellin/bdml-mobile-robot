...
class HebiRobot(object):
  ...
  def __init__(self, family='X8-9', names=['X-80749']):
    self.position = 0.0   # variable for sensor position reading
  ...
  def run_loop(self):
    ...
    while True:
      ...
      self.feedback_pub.publish(self.position)
  ...
  def get_actuator_feedback(self, feedback):
    ...
    Args:
      feedback: hebi.GroupFeedback instance contains position and velocity
